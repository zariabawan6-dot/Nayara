...
const CartPage = () => {
  ...
  const deliveryCharges = 0; // delivery is always free now
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16">
          ...
          <div className="lg:col-span-7 xl:col-span-8 space-y-8">
            ...
            <div className="bg-white rounded-sm shadow-sm border border-[#E5E7EB] overflow-hidden p-6 md:p-10 space-y-8">
              {cart.map((item) => (<div key={`${item.id}-${item.size}-${item.color}`} className="flex flex-col sm:flex-row gap-8 pb-8 border-b border-[#E5E7EB] last:border-0 last:pb-0">
                  ...
                  <div className="flex-1 flex flex-col justify-between">
                    ...
                    <div className="flex justify-between items-end mt-6">
                      <div className="flex items-center border border-[#E5E7EB] rounded-sm bg-white">
                        <button onClick={() => updateQuantity(item.id, item.size, (item.quantity || 1) - 1, item.color)} className="p-2 hover:bg-gray-50 text-gray-500 transition-colors" disabled={item.quantity <= 1}>
                          <Minus size={14} />
                        </button>
                        <span className="w-10 text-center text-sm font-semibold text-[#111827] select-none">{item.quantity || 1}</span>
                        <button onClick={() => updateQuantity(item.id, item.size, (item.quantity || 1) + 1, item.color)} className="p-2 hover:bg-gray-50 text-gray-500 transition-colors">
                          <Plus size={14} />
                        </button>
                      </div>
                      <button onClick={() => removeFromCart(item.id, item.size, item.color)} className="text-gray-400 hover:text-red-600 flex items-center gap-1 text-[10px] font-semibold uppercase tracking-widest transition-colors">
            ...
            <div className="bg-white rounded-sm shadow-md border border-[#E5E7EB] p-8 md:p-10 sticky top-24">
              ...
              <div className="space-y-4 text-sm text-gray-600 font-body">
                ...
                <div className="flex items-center gap-2 text-[11px] font-semibold text-green-600 uppercase tracking-widest">
                  <Truck size={13} /> Free shipping on all orders! 🎉
                </div>
              </div>
